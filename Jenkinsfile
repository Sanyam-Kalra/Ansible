       ...
       stage('Copy data'){
        ...
        stage('Configure ansible'){
            ...
            steps {
                ...
                IP=$(terraform output -json public_config_ec2 | jq -s -r '.[]')
                echo $IP
                ssh -i "/var/lib/jenkins/haproxy.pem" -o StrictHostKeyChecking=no -tt ubuntu@$IP "sudo apt-get update -y && sudo apt-add-repository ppa:ansible/ansible -y && sudo apt-get install ansible -y"
                '''
            }
        }
        stage('Role setup'){
